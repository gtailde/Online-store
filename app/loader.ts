...
const loadFilters = () => {
  ...
  for (let i = 0; i < allCategories.length; i++) {//массив по созданию инпутов 
    ...
    input.addEventListener('click', () => {
      if (input.checked) {
        ...
        list.push(...categEl);
      } else { console.log("false"); list = list.filter(item => allCategories[i] !== item.category); if (list.length === 0) { blblblb.innerHTML = ""; CardProd() } }
      createCard(list)
    }
    )
  }
  ...
  for (let i = 0; i < allCategories.length; i++) {//массив по созданию инпутов 
    ...
    console.log(allManufacturers[i])
    // input.nextSibling?.nodeValue
    input.addEventListener('click', () => {
      if (input.checked) {
        console.log("true")
        ...
        list.push(...categEl);
      } else { console.log("false"); list = list.filter(item => allManufacturers[i] !== item.brand); if (list.length === 0) { blblblb.innerHTML = ""; CardProd() } }
      createCard(list)
    }
    )
  }
